...
public class Algogenetique_AlphaBeta_V2 {
	...
	public Algogenetique_AlphaBeta_V2(int[] min, int[] max, int x_nb_ia_30, ArrayList<IA> Joueurs_test) {
		this.min = min;
		this.max = max;
	...
	public ArrayList<Integer[]> work(int nbIte, int prob_mutation, int nb_max_mutation) throws IOException {
		...
		for (int i = 0; i < nbIte - 1; i++) {
			...
			for (int j = 0; j < 30 * x_nb_ia_30; j++) {
				Ia_Alpha_Beta_V2_Modulable J = new Ia_Alpha_Beta_V2_Modulable(IAs.get(j)[0], IAs.get(j)[1],
						IAs.get(j)[2], IAs.get(j)[3], IAs.get(j)[4], IAs.get(j)[5], IAs.get(j)[6], IAs.get(j)[7],
						IAs.get(j)[8], IAs.get(j)[9], IAs.get(j)[10], IAs.get(j)[11], IAs.get(j)[12], IAs.get(j)[13],
						IAs.get(j)[14], IAs.get(j)[15], IAs.get(j)[16], IAs.get(j)[17], IAs.get(j)[18], IAs.get(j)[19],
						IAs.get(j)[20], IAs.get(j)[21], IAs.get(j)[22], IAs.get(j)[23], IAs.get(j)[24], IAs.get(j)[25],
						IAs.get(j)[26], IAs.get(j)[27]);
				Joueurs.add(J);
			}
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter("resultatsV2.txt"));
	...
	private ArrayList<Integer[]> reproduction(ArrayList<Integer[]> IAs_int, int prob_mutation, int nb_max_mutation) {
		...
		for (int i = 0; i < 14 * x_nb_ia_30; i++) {
			...
			int ran3 = random.nextInt(28);
			int ran4 = random.nextInt(100);
			int ran5 = random.nextInt(100);
			Integer[] IA1 = new Integer[28];
			Integer[] IA2 = new Integer[28];
			for (int j = 0; j < 28; j++) {
				if (j < ran3) {
